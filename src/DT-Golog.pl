...
/*:- set_flag(all_dynamic, on).SKHAN */     /* that you load this file first! */
:- set_flag(print_depth,500).
/* :- pragma(debug).  CHANGE 1 - remove this */

:- discontiguous(bestDo/6). /*SKHAN*/

:- op(900, fy, [not]). /* Define not   CHANGE 2  */
...
/* Define semantics of not CHANGE 3 */
(not X) := (\+ X).

/* Fix cputime CHANGE 4 (Problem - need to make it better!) */
cputime(5).
...
bp(E,H,Pol,Util,Prob,_) :- integer(H), H >= 0, /* CHANGE 6 - printf -> swritef!!! can't make it work though. */
